...
<div class="screen">
  ...
  <div class="scroll init-scroll">
    ...
    <div class="init-cols" class:single={v.yourRole === 'detective'}>
      <div class="init-left">
      ...
      <div class="init-right">
        {#if v.yourRole !== 'detective'}
          <p class="dim" style="font-size:.8rem;margin:0 0 8px">তোমার টেবিলের কার্ড · Your face-up cards <span style="font-size:.66rem">— hold to zoom</span></p>
...
<style>
  ...
  .init-cols{display:flex;gap:26px;align-items:center;justify-content:center;width:100%;max-width:940px;margin-top:12px}
  /* detective has no cards — role card alone, centred in a single column */
  .init-cols.single{flex-direction:column;gap:12px}
  .init-left{flex:0 0 auto}
  ...
  .flip-card{width:clamp(150px,40vw,190px);aspect-ratio:5/7;perspective:900px}
