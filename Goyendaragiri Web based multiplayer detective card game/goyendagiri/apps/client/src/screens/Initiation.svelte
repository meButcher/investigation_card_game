...
<div class="screen">
  ...
  <div class="scroll init-scroll">
      ...
      <div class="init-cols">
        ...
        <div class="init-right">
          <div class="ready-bar panel">
            <span class="ready-title">{$t('প্রস্তুত? · Ready — ')}{v.seats.filter(s => s.ready).length}/{v.seats.length}</span>
            {#each v.seats as s}
          ...
          <p class="dim" style="font-size:.8rem;margin:10px 0 8px">{$t('তোমার টেবিলের কার্ড · Your face-up cards ')}<span style="font-size:.66rem">— hold to zoom</span></p>
          <div class="card-grid" style="--k:{v.settings.difficulty}">
    ...
    <button class="btn gold" style="margin-top:20px;min-width:220px" disabled={amReady} on:click={() => send('ready')}>
      {amReady ? $t('✓ অপেক্ষা করছি · waiting for others') : $t('✓ প্রস্তুত · I\'m ready')}
    </button>
    </div>
  </div>
</div>
...
<style>
  ...
  /* margin:auto = vertically centred when short, still scrollable when tall */
  .init-center{margin:auto;display:flex;flex-direction:column;align-items:center;width:100%}
  .ready-bar{width:fit-content;max-width:940px;display:flex;flex-wrap:wrap;gap:6px 8px;align-items:center;padding:8px 12px}
  ...
  .init-cols{display:flex;gap:26px;align-items:stretch;justify-content:center;width:100%;max-width:940px;margin-top:12px}
  /* detective has no cards — role card alone, centred in a single column */
  .init-cols.single{flex-direction:column;gap:12px;align-items:center}
  .init-left{flex:0 0 auto;display:flex;align-items:stretch}
  .init-right{flex:1;min-width:0;display:flex;flex-direction:column;align-items:center}
  .init-right .ready-bar{width:100%}
  .card-grid{display:grid;grid-template-columns:repeat(var(--k,4),max-content);gap:10px;justify-content:center}
  @media (max-width:1023px){
    .init-cols{flex-direction:column;gap:12px}
    .card-grid{display:flex;flex-wrap:wrap;justify-content:center;order:3}
    .init-cols .flip-card{min-height:0;aspect-ratio:5/7;height:auto}
    /* mobile order: ready bar → role card (centred) → cards */
    .init-cols:not(.single) .init-right{display:contents}
    .init-cols:not(.single) .ready-bar{order:0;width:100%}
    .init-cols:not(.single) .init-left{order:1;width:100%;justify-content:center}
    .init-cols:not(.single) .init-right > p{order:2}
  }
  .init-cols.single .flip-card{min-height:0;aspect-ratio:5/7}
  .flip-card{width:clamp(170px,22vw,230px);min-height:100%;aspect-ratio:auto;perspective:900px}
