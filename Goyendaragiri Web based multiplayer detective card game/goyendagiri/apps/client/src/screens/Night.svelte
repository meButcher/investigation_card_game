...
<div class="screen night-bg">
  ...
  <div class="scroll" style="display:flex;flex-direction:column;align-items:center;padding:22px 16px">
    ...
    {#if role === 'detective'}
      ...
      <div style="display:flex;gap:14px;flex-wrap:wrap;justify-content:center;max-width:900px;width:100%">
        <div class="panel" style="flex:1.2;min-width:280px">
          <h3>{$t('🌙 রাতের ধারা · Night script — you set the pace')}</h3>
          {#each beats as b, i}
          ...
          {#if v.phase === 'nightIntro' && !n.awaitingReveal}
            {#if !nightDone && nextBeat}
              <button class="btn gold big" style="margin-top:10px" on:click={nextBeat.action}>📣 {$t(nextBeat.label)}</button>
            {:else if nightDone}
              <button class="btn gold big" style="margin-top:10px" on:click={() => send('beginDay')}>{$t('🌅 তদন্ত শুরু · Begin investigation')}</button>
            {/if}
          ...
          {:else if v.phase === 'nightKiller'}
            <p class="dim" style="font-size:.75rem;margin-top:10px">{$t('খুনী কার্ড বাছছে · The killer is choosing…')}</p>
            {#if v.secret.solution}
              <button class="btn gold big" style="margin-top:8px" on:click={() => send('dismiss')}>{$t('🙈 চোখ বন্ধ করাও · Dismiss the killer')}</button>
            {/if}
          {:else if v.phase === 'nightAccomplice' || v.phase === 'nightWitness'}
            <button class="btn gold big" style="margin-top:10px" on:click={() => send('dismiss')}>{$t('🙈 বিদায় দাও · Dismiss')}</button>
          {/if}
        </div>
        {#if v.secret.solution && murdererName}
          <div class="panel" style="flex:1;min-width:240px;border-color:var(--danger)">
            <h3 style="color:var(--danger)">{$t('🗂 কেস মেমো · Case memo')}</h3>
    ...
    {:else if v.night.awaitingYou}
      ...
      <button class="btn gold" style="min-width:220px" on:click={() => send('revealAck')}>{$t('👁 নিজেকে প্রকাশ করো · Reveal')}</button>

    {:else if v.phase === 'nightKiller' && role === 'murderer'}
      <!-- killer's linked pick view -->
      <h2 class="goldtext">{$t('তোমার অপরাধ সাজাও · Pick the solution')}</h2>
      ...
      <p class="dim" style="font-size:.72rem;margin-bottom:6px">{$t('তোমার প্রমাণ · Your evidence — pick 1')}</p>
      <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center">
        {#each me.evidence as c}
          <Card card={c} size="big" selected={pickEv === c.id} on:click={() => pickEv = c.id} />
        {/each}
      </div>
      <p class="dim" style="font-size:.72rem;margin:12px 0 6px">{$t('খুনের পদ্ধতি · Your means — pick 1')}</p>
      <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center">
        {#each me.means as c}
      ...
      <button class="btn gold" style="margin-top:18px;min-width:220px" disabled={!canConfirm || confirmedCurrent}
        ...
        {confirmedCurrent ? $t('✓ নিশ্চিত হয়েছে · Confirmed') : sol ? $t('🔄 বদলে দাও · Update choice') : $t('নিশ্চিত করো · Confirm')}
      </button>
      {#if sol}
        <p class="dim" style="font-size:.7rem;margin-top:8px">{$t('গোয়েন্দা বিদায় না দেওয়া পর্যন্ত কার্ড বদলাতে পারবে · tap other cards to change until the Detective dismisses you')}</p>
      {/if}

    {:else if v.phase === 'nightAccomplice' && role === 'accomplice' && v.secret.solution}
      <h2 class="goldtext">{$t('🗂 কেস মেমো · Your case memo')}</h2>
    ...
    {:else if v.phase === 'nightWitness' && role === 'witness' && murdererName}
      ...
      <p style="margin:10px 0;font-size:1.1rem"><b>{murdererName}</b> <span class="dim">{$t('— খুনী · is the murderer')}</span></p>
