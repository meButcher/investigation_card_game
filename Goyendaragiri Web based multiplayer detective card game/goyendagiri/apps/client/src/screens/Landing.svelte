...
<div class="screen" style="align-items:center;justify-content:center">
  <div class="panel" style="width:min(420px,92vw)">
    ...
    <button class="btn gold big" on:click={create} disabled={busy || !name.trim()}>
      {$t('🕯 নতুন রুম · Create room')}
    </button>

    <div class="dim" style="text-align:center;margin:12px 0;font-size:.75rem">{$t('— অথবা · or —')}</div>

    <div style="display:flex;gap:8px">
      <input class="input" bind:value={code} placeholder={$t('রুম কোড · room code')} style="text-transform:none" />
    ...
    {/if}
    <p class="dim" style="font-size:.68rem;margin-top:16px">{$t('🎙 ভয়েসের জন্য Discord কল ব্যবহার করো · use a Discord call for voice — this game is the shared board.')}</p>
  </div>
</div>
